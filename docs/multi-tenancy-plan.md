...
### 1.5 Database backstops (independent of the app layer)
- Migration `20260819130000_same_org_fk_triggers` (generated by `packages/db/scripts/generate-same-org-triggers.ts`): a `BEFORE INSERT OR UPDATE` trigger per tenant→tenant FK (46 edges) raises `cross-tenant reference` when the referenced row belongs to another organization, and `organizationId` is immutable on every tenant table (46). Regenerate + new migration whenever a relation between tenant models is added.
- Compound keys per org: `TrackingCounter @@id([organizationId,key])`, `TrackedVisitor @@id([organizationId,id])`, `TrackedPageDaily @@id([organizationId,day,host,path])`, `FormSubmission @@unique([organizationId,dedupeKey])`; `SsoProvider.organizationId` → FK to Organization.
- Extension hardening: update data cannot re-home a row; nested `createMany` rows are scalar-only; `_count.select.<list>` scoped; legacy single-field uniques and `@@id` compound keys rewritten with the org pinned.
